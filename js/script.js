...
if (spollersArray.length > 0) {
	...
	function hideSpollerBody(spollersBlock) {
		const spollerActivateTitle = spollersBlock.querySelector('[data-spoller]._active');
		if (spollerActivateTitle) {
			spollerActivateTitle.classList.remove('_active');
			_slideUp(spollerActivateTitle.nextElementSibling, 500);
		}
	}
}
...
let _slideUp = (target, duration = 500) => {
	if (!target.classList.contains('_slide')) {
		...
		target.style.transitionDuratin = duration + 'ms';
